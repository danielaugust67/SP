    ...
    local Support = {
        Webhook = (typeof(HttpRequest) == "function"),
    ...
    local Remotes = {
        ...
        EasterBuy = GetRemote(RS, "Remotes.EasterMerchantRemotes.PurchaseEasterMerchantItem"),
    ...
    local Modules = {
        ...
        EasterConfig = GetSafeModule(RS.Modules, "EasterMerchantConfig"),
    ...
    local NPCs = {
        Merchant = {
            ...
            Easter = GetServiceNPC("EasterMerchantNPC"),
        }
    }
    ...
    local UI = {
        Merchant = {
            ...
            Easter = PGui:FindFirstChild("EasterMerchantUI"),
        }
    }
    ...
    local IslandCrystals = {
        ...
        ["Easter"] = workspace:FindFirstChild("EasterIsland") and workspace.EasterIsland:FindFirstChild("SpawnPointCrystal_Easter"),
        ...
        ["World"] = workspace:FindFirstChild("WorldIsland") and workspace.WorldIsland:FindFirstChild("SpawnPointCrystal_World"),
    }
    ...
    local Tables = {
        ...
        IslandList = {"Starter", "Jungle", "Desert", "Snow", "Sailor", "Shibuya", "HuecoMundo", "Boss", "Dungeon", "Shinjuku", "Easter", "Slime", "Academy", "Judgement", "SoulSociety", "World"},
    ...
    if not table.find(Tables.BossList, "CosmicBeingBoss_Normal") then
        table.insert(Tables.BossList, "CosmicBeingBoss_Normal")
        Shared.BossTIMap["CosmicBeingBoss_Normal"] = "World"
    end
    table.sort(Tables.BossList)
    ...
    local function IsValidWebhookURL(url)
        return type(url) == "string" and url ~= "" and url:find("discord.com/api/webhooks/", 1, true) ~= nil
    end

    local function ParseStatusCode(response)
        if type(response) ~= "table" then return nil end
        return tonumber(response.StatusCode or response.Status or response.status_code or response.status)
    end

    local function ParseRetryAfter(response)
        if type(response) ~= "table" then return nil end
        ...
        local retryAfter = tonumber(headers["Retry-After"] or headers["retry-after"])
            or tonumber(headers["X-RateLimit-Reset-After"] or headers["x-ratelimit-reset-after"])
        if not retryAfter then return nil end

        return math.clamp(retryAfter, 0.5, 15)
    end

    local function SendWebhookRequest(payload, cfg)
        cfg = cfg or {}
        ...
        local url = cfg.url or ((Options.WebhookURL and Options.WebhookURL.Value) or "")
        if not IsValidWebhookURL(url) then
            return false, "invalid-webhook-url"
        end

        local body = HttpService:JSONEncode(payload)
        local maxAttempts = math.max(1, tonumber(cfg.maxAttempts) or 3)
        local baseDelay = math.max(0.25, tonumber(cfg.baseDelay) or 0.75)
        ...
        for attempt = 1, maxAttempts do
            ...
            local statusCode = ok and ParseStatusCode(response) or nil
            if ok and (statusCode == nil or (statusCode >= 200 and statusCode < 300) or statusCode == 204) then
                return true, statusCode, response
            end

            if attempt < maxAttempts then
                local waitTime = baseDelay * attempt
                if statusCode == 429 then
                    waitTime = ParseRetryAfter(response) or math.max(waitTime, 1.5)
                end
                task.wait(math.clamp(waitTime, 0.25, 15))
            else
                local reason = statusCode and ("status-" .. tostring(statusCode)) or "request-failed"
                if cfg.notifyOnFail and Library and Library.Notify then
                    Library:Notify("Webhook failed (" .. reason .. ")", 4)
                end
    ...
    local function PostToWebhook(isManual)
        ...
        task.spawn(function()
            ...
            elseif isManual == true then
                Library:Notify("Webhook test failed.", 4)
            end
        end)
    end
    ...
    local function Func_AutoRejoin()
        ...
        if Support.QueueOnTeleport then
            ...
            Connections.TeleportQueue = Plr.OnTeleport:Connect(function(state)
                if Toggles.AutoExecuteTeleport and Toggles.AutoExecuteTeleport.Value then
                    local code = Options.AutoExecuteStr.Value
                    if code and code ~= "" then
                        pcall(queue_on_teleport, code)
                    end
    ...
    local BossDefeatState = {
        Watched = {},
        LastNotifyAt = {},
        Hooked = false,
    }

    local function GetSelectedBossDisplayName(npcName)
        local selected = (Options.SelectedBosses and Options.SelectedBosses.Value) or {}
        for bossDisplayName, enabled in pairs(selected) do
            if enabled and IsStrictBossMatch(npcName, bossDisplayName) then
    ...
    local function SendBossDefeatedWebhook(bossDisplayName, npcName, forceSend)
        ...
        local payload = {
            ["embeds"] = {{
                ["title"] = "Boss Defeated",
                ["description"] = string.format("Selected boss defeated: **%s**", bossDisplayName),
                ["color"] = 65280,
                ["fields"] = {
                    { ["name"] = "Boss", ["value"] = "`" .. tostring(npcName) .. "`", ["inline"] = true },
    ...
    local function NotifyBossDefeated(model)
        if Toggles.NotifyBossDefeatOnlyWhenFarm and Toggles.NotifyBossDefeatOnlyWhenFarm.Value and (not Toggles.BossesFarm or not Toggles.BossesFarm.Value) then
            return
        end

        local bossDisplayName = GetSelectedBossDisplayName(model.Name)
        if not bossDisplayName then return end

        local now = tick()
        local key = bossDisplayName:lower()
        if BossDefeatState.LastNotifyAt[key] and (now - BossDefeatState.LastNotifyAt[key]) < 5 then
            return
        end
        BossDefeatState.LastNotifyAt[key] = now

        if not Toggles.NotifyBossDefeat or Toggles.NotifyBossDefeat.Value then
            Library:Notify("Boss defeated: " .. bossDisplayName, 5)
        end

        SendBossDefeatedWebhook(bossDisplayName, model.Name)
    end

    local function CleanupBossWatch(model)
        local entry = BossDefeatState.Watched[model]
        if not entry then return end

        if entry.Died then entry.Died:Disconnect() end
        if entry.Ancestry then entry.Ancestry:Disconnect() end
        if entry.ChildAdded then entry.ChildAdded:Disconnect() end
        BossDefeatState.Watched[model] = nil
    end

    local function WatchBossModel(model)
        if not model or not model:IsA("Model") then return end
        if BossDefeatState.Watched[model] then return end
        ...
        if not human then
            ...
            childConn = model.ChildAdded:Connect(function(child)
                if child:IsA("Humanoid") then
                    ...
                    BossDefeatState.Watched[model] = nil
                    WatchBossModel(model)
                end
            end)

            BossDefeatState.Watched[model] = { ChildAdded = childConn }
            return
        end

        local diedConn = human.Died:Connect(function()
            NotifyBossDefeated(model)
        end)

        local ancestryConn = model.AncestryChanged:Connect(function(_, parent)
            if not parent then
                CleanupBossWatch(model)
            end
        end)

        BossDefeatState.Watched[model] = {
            Died = diedConn,
            Ancestry = ancestryConn,
        }
    end

    local function SetupBossDefeatNotifier()
        if BossDefeatState.Hooked then return end
        BossDefeatState.Hooked = true

        for _, mob in ipairs(PATH.Mobs:GetChildren()) do
            if mob:IsA("Model") and mob.Name:find("Boss") and not table.find(Tables.MiniBossList, mob.Name) then
                WatchBossModel(mob)
            end
        end

        Connections.BossSpawnWatcher = PATH.Mobs.ChildAdded:Connect(function(child)
            if child:IsA("Model") and child.Name:find("Boss") and not table.find(Tables.MiniBossList, child.Name) then
                WatchBossModel(child)
            end
        end)
    end
    ...
    local function ExecuteFarmLogic(target, island, farmType)
        ...
        if posType == "Center" then
            finalDestination = CFrame.new(finalPos) -- Tanpa referensi rotasi LookAt agar tidak glitch
        else
            finalDestination = CFrame.lookAt(finalPos, targetPos)
        end
    ...
    local TB_Tabs = {
        ...
        Merchant = {
            ...
            T3 = TB.Misc.Left.Merchant:AddTab("Easter"),
        },
        Misc1 = {
    ...
    GB.Webhook.Left.Config:AddToggle("NotifyBossDefeat", {
        Text = "Notify Selected Boss Defeated",
        Default = true,
    })

    GB.Webhook.Left.Config:AddToggle("NotifyBossDefeatOnlyWhenFarm", {
        Text = "Only Notify While BossesFarm ON",
        Default = false,
    })

    GB.Webhook.Left.Config:AddToggle("NotifyBossDefeatWebhook", {
        Text = "Send Boss Defeat to Webhook",
        Default = false,
    ...
    local function RefreshBossLists()
        Modules.BossConfig = GetSafeModule(RS.Modules, "BossConfig") or {Bosses = {}}
        Modules.TimedConfig = GetSafeModule(RS.Modules, "TimedBossConfig")
        
        table.clear(Tables.BossList)
        table.clear(Shared.BossTIMap)
        table.clear(Tables.AllBossList)
        ...
        if Modules.TimedConfig and Modules.TimedConfig.Bosses then
            for internalName, data in pairs(Modules.TimedConfig.Bosses) do
                ...
                if data.spawnLocation == "Hueco Mundo Island" then tpName = "HuecoMundo" end
                if data.spawnLocation == "Judgement Island" then tpName = "Judgement" end
                if data.spawnLocation == "World Island" then tpName = "World" end
        ...
        if Options.SelectedBosses then Options.SelectedBosses:SetValues(Tables.BossList) end
        if Options.SelectedBuildPity then Options.SelectedBuildPity:SetValues(Tables.AllBossList) end
        if Options.SelectedUsePity then Options.SelectedUsePity:SetValues(Tables.AllBossList) end
        if Options.SelectedAltBoss then Options.SelectedAltBoss:SetValues(Tables.AllBossList) end
        
        if Library then Library:Notify("Boss list has been refreshed!", 3) end
    end

    TB_Tabs.Autofarm.T2:AddButton("Refresh Boss List", function()
        RefreshBossLists()
    end)
    ...
    TB_Tabs.Dungeon.T2:AddToggle("DungeonAutofarm", {
        ...
        GB.Player.Left.Server:AddInput("PrivateServerCode", {
            Numeric = false,
            Finished = true,
    ...
    TB_Tabs.Waypoint.T3:AddButton("Teleport to Sea 2", function()
        local char = Plr.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        ...
        if door then
            ...
            local prompt = door:FindFirstChildOfClass("ProximityPrompt")
            if prompt then
                -- Memicu ProximityPrompt secara instan mengabaikan waktu "tekan dan tahan" (HoldDuration)
                fireproximityprompt(prompt)
                Library:Notify("Requesting to enter Sea 2...", 3)
            else
                Library:Notify("ProximityPrompt not found on the door!", 3)
            end
        else
            Library:Notify("Sea 2 Door not found! Please wait for it to load.", 3)
        end
    end)
    ...
    EasterLabel = TB_Tabs.Merchant.T3:AddLabel("Hearts: N/A")

    TB_Tabs.Merchant.T3:AddDropdown("SelectedEasterMerchantItems", {
        Text = "Select Item (s)",
        Values = Tables.EasterMerchantList,
        Default = nil,
        Multi = true,
        Searchable = true,
    })

    EasterPriceLabel = TB_Tabs.Merchant.T3:AddLabel("Price: N/A")

    TB_Tabs.Merchant.T3:AddToggle("AutoEasterMerchant", {
        Text = "Auto Buy Selected Items",
        Default = false,
    })
